...
## 2024-05-20 - ARIA Roles for Dynamic Logs and Modals
**Learning:** Dynamically updating areas like activity logs are not announced by screen readers automatically, and visual modal overlays lack structural meaning without proper ARIA attributes.
**Action:** Always add `role="log"` and `aria-live="polite"` to dynamically updating text containers. For all modal dialogs, ensure `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` (linked to a heading ID) are present on the container element.
